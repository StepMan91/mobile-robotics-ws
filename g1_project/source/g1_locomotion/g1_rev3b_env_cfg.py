...
def illegal_tilt(env, limit: float = 0.4): # Relaxed from 0.26 to 0.4 (~23 deg)
...
class EventCfg:
    ...
    # Reset Pose: Facing Stairs
    reset_base = EventTerm(
        func=mdp.reset_root_state_uniform,
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("robot"),
            "pose_range": {"x": (0.5, 1.5), "y": (-0.2, 0.2), "yaw": (-0.05, 0.05)}, 
            "velocity_range": {},
        },
    )
...
class TerminationsCfg:
    ...
    check_tilt = TermTerm(func=illegal_tilt, params={"limit": 0.8}) # Relaxed to ~45 deg for climbing
    
    check_fly = TermTerm(
            func=no_ground_contact,
            params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_ankle_.*")},
            time_out=0.2, # Die if flying
    )
    
    base_low = TermTerm(func=mdp.root_height_below_minimum, params={"minimum_height": 0.35})

@configclass
class G1Rev3bEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the G1 Climbing environment (Rev3b)."""
