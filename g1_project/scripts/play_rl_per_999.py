...
# play_rl_per_999.py
# Visualization script for PER trained policy - Checkpoint 999
...
def main():
    ...
    parser.add_argument("--num_envs", type=int, default=1) # Low num envs for visual
    # HARDCODED DEFAULTS FOR 999
    parser.add_argument("--load_run", type=str, default="v2_per_long", help="Name of run folder")
    parser.add_argument("--checkpoint", type=str, default="model_999.pt")
    args = parser.parse_args()

    # Config
    env_cfg = G1StairsEnvCfg()
    
    # Check if stairs USD exists
    if hasattr(env_cfg.scene.terrain, "usd_path"):
        usd_path = env_cfg.scene.terrain.usd_path
        if not os.path.exists(usd_path):
            print(f"[WARNING] Stairs USD not found at: {usd_path}")
            print(f"[INFO] Attempting to generate it...")
            # Optional: Call generation script?
            # For now just warn
    ...
    # FIX: Checkpoints are flat in log_dir because train script passed log_dir directly
    resume_path = os.path.join(log_dir, args.checkpoint)
    if not os.path.exists(resume_path):
        # Fallback to structured path if it exists
        resume_path_structured = os.path.join(log_dir, "g1_stairs_per", args.load_run, "nn", args.checkpoint)
        if os.path.exists(resume_path_structured):
            resume_path = resume_path_structured
    ...
    print("[INFO] Starting Playback 999...")
