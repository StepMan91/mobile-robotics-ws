...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, default="model_2200.pt") # Rev3 2200
    ...
    # Disable random initialization for visualization to see consistent behavior
    # Start facing stairs
    env_cfg.events.reset_base.params["pose_range"] = {"x": (1.0, 1.0), "y": (0.0, 0.0), "yaw": (0.0, 0.0)}

    # OVERRIDE STIFFNESS to Standard G1 (User requested 800, but it locks the robot)
    # Trying 150 to see if it moves.
    print("[INFO] Overriding Stiffness to 150.0 (Standard G1) for smoother playback.")
    for key in env_cfg.scene.robot.actuators.keys():
         env_cfg.scene.robot.actuators[key].stiffness = 150.0
         env_cfg.scene.robot.actuators[key].damping = 5.0
    ...
    # RSL-RL Config (Minimal for loading - must match training structure)
    ppo_config = {
        "seed": 42,
        ...
        "max_iterations": 7000,
        "save_interval": 200,
        "experiment_name": "climb_per_rev3_7k",
        "run_name": "run_001",
        "algorithm": { "class_name": "PPO", "value_loss_coef": 1.0, "use_clipped_value_loss": True, "clip_param": 0.2, "entropy_coef": 0.01, "num_learning_epochs": 5, "num_mini_batches": 4, "learning_rate": 3e-4, "schedule": "adaptive", "gamma": 0.99, "lam": 0.95, "desired_kl": 0.01, "max_grad_norm": 1.0 },
        "policy": { "class_name": "ActorCritic", "init_noise_std": 1.0, "actor_hidden_dims": [128, 64, 32], "critic_hidden_dims": [128, 64, 32], "activation": "elu" }
    }
    
    # Correct Log Dir for Rev3
    ...
    # Standard load failed due to log_std mismatch. Manual patch:
    ...
    while simulation_app.is_running():
        ...
        with torch.no_grad():
            ...
            if count % 50 == 0:
                print(f"Frame {count}:")
                # print(f"  Actions: Mean={actions.mean().item():.3f}, Std={actions.std().item():.3f}, Max={actions.abs().max().item():.3f}")
                # print(f"  Obs    : Mean={obs.mean().item():.3f}, Std={obs.std().item():.3f}")
                # Printing simpler stats to avoid accidental errors
